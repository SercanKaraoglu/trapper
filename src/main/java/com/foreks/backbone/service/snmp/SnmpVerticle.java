...
public class SnmpVerticle extends BusModBase implements CommandResponder, Handler<Message<JsonObject>> {
    public static final String                      IPV4_REGEXP = "^\\s*[0-2]?\\d?(?<!2[6-9])\\d(?<!25[6-9])(\\.[0-2]?\\d?(?<!2[6-9])\\d(?<!25[6-9])){3}\\s*$";
    private static final List<SnmpVerticle>         modules     = new Vector<>();
    private Snmp                                    snmp;
    private final Map<TransportMapping, JsonObject> mappings    = new HashMap<>();
    private String                                  address;

    private static List<SnmpVerticle> getModulesByConfig(final JsonObject config) {
        final List<SnmpVerticle> list = new ArrayList<>();
        for (final SnmpVerticle module : modules) {
    ...
    private static List<SnmpVerticle> getModulesBySnmp(final Snmp snmp) {
        final List<SnmpVerticle> list = new ArrayList<>();
        for (final SnmpVerticle module : modules) {
    ...
    public void start() {
        super.start();
        this.address = this.container.config().getString("address", "com.foreks.snmp.agent");
        synchronized (modules) {
            final List<SnmpVerticle> exists = getModulesByConfig(this.config);
            if (exists.isEmpty()) {
                try {
                ...
                for (final SnmpVerticle module : exists) {
                    if (module.snmp != null) {
...
=======
        final Address address = GenericAddress.parse(config.getObject("transport-mapping").getString("address", "udp:0.0.0.0/10161"));
>>>>>>> 0114094e0a3ada02ade2e350c37503359c46dba7
    ...
    public void handle(final Message<JsonObject> message) {
        ...
        final ResponseListener listener = new ResponseListener() {
            ...
            public void onResponse(final ResponseEvent event) {
                ...
                final PDU myPdu = new PDU(response) {
                    private static final long serialVersionUID = -5998938285922419197L;
        ...
        try {
            if (pdu.getType() == PDU.V1TRAP) {
                sendOK(message, null);
                ((PDUv1) pdu).setTimestamp(MILLISECONDS.toSeconds(ManagementFactory.getRuntimeMXBean().getUptime()));
                this.snmp.trap((PDUv1) pdu, target);
            } else {
